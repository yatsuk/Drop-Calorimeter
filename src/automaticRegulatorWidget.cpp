...
void AutomaticRegulatorWidget::setTotalDuration(double duration){
    totalDurationLabel->setText(tr("Общая продолжительность ")+QTime().addSecs(duration*60).toString("hh:mm:ss"));
}

void AutomaticRegulatorWidget::setRemainingTime(double time){
    remainingTimeLabel->setText(tr("Оставшееся время ")+QTime().addSecs(time*60).toString("hh:mm:ss"));
}
...
void AutomaticRegulatorWidget::updateTable(){
    ...
    for (int i=0;i<temperatureSegments->segmentCount();++i){
        ...
        QTableWidgetItem * duration = new QTableWidgetItem(QTime().addSecs(temperatureSegments->segment(i)->duration()*60).toString("hh:mm:ss"));
